...
/*
   ###############################################################
   #                 Iteration 6: Details                        #
   #      Addressed a discovered logic loop bug related to the   #
   #      Wi-Fi connect class. Should the code become locked     #
   #      into the while loop, the Wi-Fi connection will be      #
   #      dropped and reinitialized                              #
   #                                                             #
   ###############################################################
*/
...
//in the event that the ESP32 fails to connect to the AP, toggle Wi-Fi off and on
int AP_connect_loop_num = 0;
  while (WiFi.status() != WL_CONNECTED) {
    if(AP_connect_loop_num > 20){
      LCD_Display("ERROR:", 1500, 2);
      LCD_Display("reseting connection", 1500, 1);
      WiFi.mode(WIFI_OFF);
      delay(2000);
      WiFi.begin(mem_ssid.c_str(), mem_pass.c_str());
      AP_connect_loop_num = 0;
    }
    else{
